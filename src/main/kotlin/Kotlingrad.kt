...
open class VFun<X : Fun<X>, E : `1`>(
    ...
    constructor(length: Nat<E>, contents: List<Fun<X>>) : this(
        length,
        contents.flatMap { it.vars }.toSet(),
        emptySet(),
        *contents.toTypedArray()
    )

    constructor(length: Nat<E>, vararg contents: Fun<X>) : this(
        length,
        contents.flatMap { it.vars }.toSet(),
        emptySet(),
        *contents
    ...
    constructor(length: Nat<E>, vararg vFns: VFun<X, E>) : this(
        length,
        vFns.flatMap { it.sVars }.toSet(),
    ...
    companion object {
        ...
        operator fun <T : Fun<T>> invoke(t0: Fun<T>, t1: Fun<T>, t2: Fun<T>): VFun<T, `3`> =
            VFun(`3`, arrayListOf(t0, t1, t2))

        operator fun <T : Fun<T>> invoke(t0: Fun<T>, t1: Fun<T>, t2: Fun<T>, t3: Fun<T>): VFun<T, `4`> =
            VFun(`4`, arrayListOf(t0, t1, t2, t3))

        operator fun <T : Fun<T>> invoke(t0: Fun<T>, t1: Fun<T>, t2: Fun<T>, t3: Fun<T>, t4: Fun<T>): VFun<T, `5`> =
            VFun(`5`, arrayListOf(t0, t1, t2, t3, t4))

        operator fun <T : Fun<T>> invoke(
            t0: Fun<T>,
            t1: Fun<T>,
            t2: Fun<T>,
            t3: Fun<T>,
            t4: Fun<T>,
            t5: Fun<T>
        ): VFun<T, `6`> = VFun(`6`, arrayListOf(t0, t1, t2, t3, t4, t5))
        ...
        operator fun <T : Fun<T>> invoke(
            ...
            t5: Fun<T>,
            t6: Fun<T>
        ): VFun<T, `7`> = VFun(`7`, arrayListOf(t0, t1, t2, t3, t4, t5, t6))
        ...
        operator fun <T : Fun<T>> invoke(
            ...
            t6: Fun<T>,
            t7: Fun<T>
        ): VFun<T, `8`> = VFun(`8`, arrayListOf(t0, t1, t2, t3, t4, t5, t6, t7))
        ...
        operator fun <T : Fun<T>> invoke(
            ...
            t7: Fun<T>,
            t8: Fun<T>
        ): VFun<T, `9`> = VFun(`9`, arrayListOf(t0, t1, t2, t3, t4, t5, t6, t7, t8))
    }
...
//    val expand: MFun<X, `1`, E> by lazy { MFun(`1`, length, this) }
    ...
    operator fun invoke(
        sMap: Map<Var<X>, X> = emptyMap(),
        vMap: Map<VVar<X, E>, VConst<X, E>> = emptyMap()
    ): VFun<X, E> =
        when (this) {
...
class SVProd<X : Fun<X>, E : `1`>(val left: Fun<X>, val right: VFun<X, E>) :
    VFun<X, E>(right.length, left.vars + right.sVars, right.vVars, *right.contents)

class VVar<X : Fun<X>, E : `1`>(override val name: String, override val length: Nat<E>, vararg val value: X) : Variable,
    VFun<X, E>(length, *value) {
    override val vVars: Set<VVar<X, *>> = setOf(this)
}

open class VConst<X : Fun<X>, E : `1`>(length: Nat<E>, override vararg val contents: SConst<X>) :
    VFun<X, E>(length, *contents)

abstract class RealVector<X : Fun<X>, E : `1`>(length: Nat<E>, override vararg val contents: SConst<X>) :
    VConst<X, E>(length, *contents)
...
sealed class `1`(override val i: Int = 1) : `0`(i) {
    companion object : `1`(), Nat<`1`>
}

sealed class `2`(override val i: Int = 2) : `1`(i) {
    companion object : `2`(), Nat<`2`>
}

sealed class `3`(override val i: Int = 3) : `2`(i) {
    companion object : `3`(), Nat<`3`>
}

sealed class `4`(override val i: Int = 4) : `3`(i) {
    companion object : `4`(), Nat<`4`>
}

sealed class `5`(override val i: Int = 5) : `4`(i) {
    companion object : `5`(), Nat<`5`>
}

sealed class `6`(override val i: Int = 6) : `5`(i) {
    companion object : `6`(), Nat<`6`>
}

sealed class `7`(override val i: Int = 7) : `6`(i) {
    companion object : `7`(), Nat<`7`>
}

sealed class `8`(override val i: Int = 8) : `7`(i) {
    companion object : `8`(), Nat<`8`>
}

sealed class `9`(override val i: Int = 9) : `8`(i) {
    companion object : `9`(), Nat<`9`>
}